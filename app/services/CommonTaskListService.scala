...
class CommonTaskListService @Inject() (appConfig: AppConfig,
  ...
  private def getTasks(cisDeductions: AllCISDeductions, taxYear: Int, journeyAnswers: Option[JourneyAnswers]): Option[Seq[TaskListSectionItem]] = {
    ...
    def getSubmissionDate(cisSource: Option[CISSource]): Long =
      cisSource
        .flatMap(
          _.cisDeductions.headOption.flatMap(
            _.periodData
    ...
    def cisTask(status: TaskStatus): Option[Seq[TaskListSectionItem]] = Some(
      Seq(
